...
### Отладчик в редакторе обработчика

Редактор поля **Текст обработчика** имеет встроенный отладчик: можно поставить точки останова и выполнить обработчик по шагам прямо в карточке потока. Переменные контекста заполняются автоматически:

- для исходящего потока `ИсходныеДанные` — ссылка из поля **Пример**;
- для входящего потока `ИсходныеДанные` — десериализованный JSON из поля **Тестовые данные**; правка поля учитывается со следующего запуска отладки.
