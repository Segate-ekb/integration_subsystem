...
| **Адрес ресурса** | `https://wms.example.com/api` |
| **Тип авторизации** | Bearer |
| **Проверять SSL** | ✅ |
| **Таймаут соединения** | 30 сек |

Учётные данные (токен, пароль и т.д.) вводятся на соответствующей странице формы и сохраняются в безопасном хранилище.

→ [Подробнее о настройке эндпоинтов](/guide/configure-endpoints)
...
Если нужно обработать ответ от API. Перечень доступных переменных — в разделе
[Подписчики](/concepts/subscribers#обработчик-ответа).
...
Если Ответ.КодСостояния = 201 Тогда
    ...
    // Сохраняем соответствие внешнего идентификатора и ссылки
    РегистрыСведений.инт_ИдентификаторыВнешнихСистем.ЗаписатьСоответствиеИдентификатораИСсылки(
        ДанныеОтвета.Получить("externalId"),
        ИсходныеДанные);
ИначеЕсли Ответ.КодСостояния >= 400 Тогда
    ...
    ЗаписьЖурналаРегистрации("WMS.Ошибка",
        ...
        ,
        ИсходныеДанные,
        ПолучитьСтрокуИзДвоичныхДанных(Ответ.Тело));
КонецЕсли;
```
...
::: warning Статус: В разработке
Отправка сообщений в RabbitMQ реализована в виде заглушки. Функционал будет добавлен в следующих версиях.
:::
...
| Реквизит | Значение |
|----------|---------|
| **Адрес ресурса** | `amqp://rabbitmq.example.com:5672/%2Fintegration` |
| **Логин** | publisher |
| **Пароль** | ****** |
