...
| `инт_ПодписчикиПодсистемаИнтеграции` | Межбазовая интеграция |
...
| `инт_АктивныеКонсьюмеры` | Активные фоновые задания Kafka-консьюмеров |
| `инт_СтатистикаОбработкиСообщений` | Статистика обработки (производительность) |
...
| `инт_МенеджерВходящихПотоков` | Управление Kafka-консьюмерами |
| `инт_КонсьюмерОчередей` | Чтение сообщений из Kafka |
| `инт_ПодписчикиОбщий` | Диспетчер вызовов типизированных подписчиков |
| `инт_ВалидаторПакетов` | Валидация по OpenAPI-схемам |
| `инт_КоннекторHTTP` | HTTP-клиент для отправки |
| `инт_РаботаСКафка` | Работа с Apache Kafka (отправка, чтение, топики) |
| `инт_РаботаСБезопаснымХранилищем` | Управление секретами в безопасном хранилище |
