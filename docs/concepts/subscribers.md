...
::: tip Настройка эндпоинтов
Учётные данные (пароли, токены, ключи) хранятся безопасно в отдельном справочнике `инт_Эндпоинты`. Поддерживаемые типы авторизации: `Анонимный`, `Basic`, `Bearer`, `Digest`, `Tls`, `ApiKey`.

→ [Управление эндпоинтами](/concepts/endpoint-management) | [Настройка эндпоинтов (практика)](/guide/configure-endpoints)
:::
...
Механизм генерации объявляет в обработчике фиксированный набор переменных:

| Переменная | Тип | Назначение | Запись |
|------------|-----|------------|--------|
| `Ответ` | `Структура` | Ответ подписчика: `КодСостояния` и `Тело` | нет |
| `ИсходныеДанные` | `ЛюбаяСсылка`, `ФиксированнаяСтруктура` | Данные, по которым формировалось сообщение | нет |
| `Результат` | `Соответствие` | Результат обработки, инициализирован пустым соответствием | да |

Объявлять свои переменные оператором `Перем` в тексте обработчика нельзя: достаточно
присвоить значение.

<!-- пример обработчика: ПостОбработкаПодписчика -->

```bsl
// Доступны: Ответ - Структура с полями КодСостояния и Тело,
// ИсходныеДанные - данные исходного сообщения, Результат - Соответствие

Если Ответ.КодСостояния = 200 Тогда
    // Парсинг JSON-ответа
    ЧтениеJSON = Новый ЧтениеJSON;
    ЧтениеJSON.УстановитьСтроку(ПолучитьСтрокуИзДвоичныхДанных(Ответ.Тело));
    ДанныеОтвета = ПрочитатьJSON(ЧтениеJSON);

    // Внешний идентификатор из ответа
    Результат.Вставить("externalId", ДанныеОтвета.Получить("externalId"));
КонецЕсли;
```

::: warning Тело ответа - двоичные данные
`Ответ.Тело` содержит двоичные данные. Для разбора текста применяйте
...
- [Управление эндпоинтами](/concepts/endpoint-management) — авторизация и безопасное хранение
