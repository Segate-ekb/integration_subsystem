...
|--------|-----|----------|
| `ПотокДанных` | СправочникСсылка.инт_ПотокиДанных | Поток обработки |
| `ВходящееСообщение` | ХранилищеЗначения | Десериализованные данные сообщения |

| Реквизит | Тип | Описание |
|----------|-----|----------|
| `ДатаСоздания` | Дата (дата и время, индексируется) | Момент регистрации сообщения в очереди |
...
Первым шагом задание вызывает `инт_СервисныеДействия.УдалитьОсиротевшиеСтатусыВходящихСообщений()` — удаляет записи `инт_ТекущийСтатусВходящихСообщений` (и историю статусов), для которых нет записи в `инт_ОчередьВходящихСообщений` или не заполнен идентификатор сообщения. Такие «осиротевшие» статусы не могут быть обработаны и раньше блокировали обработку входящей очереди. Процедуру можно вызвать и вручную. Количество удалённых записей пишется в журнал регистрации (событие `ПодсистемаИнтеграции.ОчисткаУстаревшихСообщений`).

Срок хранения задаётся реквизитом `УдалятьОбработанныеСообщенияЧерез` потока данных. Сообщение попадает в регистр `инт_СообщенияКУдалению` в момент, когда становится обработанным. Если срок хранения задали позже, чем сообщения были обработаны, дату удаления им проставляет фоновое задание. Оно запускается из формы элемента потока данных, когда поток записывают с изменённым сроком хранения.

Задание запускается только при изменении значения срока: повторная запись потока с тем же сроком регистрацию не перезапускает. Если запустить задание не удалось (в журнал регистрации попадает запись события `ПодсистемаИнтеграции.СообщенияКУдалению`), регистрацию повторяют изменением срока хранения — например, задав другое значение и вернув прежнее.
