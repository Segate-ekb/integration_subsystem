...
::: tip Назначение собираемой статистики (ТР-Ф-008)
Записи статистики синхронного пути нужны метрике `pde_incoming_duration_seconds`, которая охватывает
оба режима обработки в разрезе лейблов `flow_name` и `status`. Отдельная метрика
`pde_sync_incoming_duration_seconds` упразднена: её наблюдения были строгим подмножеством
наблюдений входящей метрики. Сам сбор статистики, описанный этой спецификацией, сохраняется
без изменений.
:::
