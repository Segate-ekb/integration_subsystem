...
Функция ПолучитьСообщенияКОтправке(КоличествоСообщенийВПачке, ТипПодписчика) Экспорт
	...
	ОтборПотипуПодписчика ="";
	Запрос = Новый Запрос;
	Если ЗначениеЗаполнено(ТипПодписчика) Тогда
		ОтборПоТипуПодписчика = "И (инт_Подписчики.ТипПодписчика = &ТипПодписчика)";
    ...
    Запрос.Текст = СтрШаблон("ВЫБРАТЬ %1
                   ...
                   |ИЗ
                   |    ПромежуточныйИтог КАК ПромежуточныйИтог", ПодстрокаОграничения, ОтборПоТипуПодписчика);
