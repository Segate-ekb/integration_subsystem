...
//
// Параметры:
//  ИдентификаторСообщения     -  УникальныйИдентификатор, Строка  - Уникальный идентификатор сообщения. Обязателен к заполнению.
...
//
Процедура ЗаписатьСтатусСообщения(Знач ИдентификаторСообщения, СтатусСообщения, ТекстОшибки="") Экспорт
    ИдентификаторСообщения = ПроверенныйИдентификаторСообщения(ИдентификаторСообщения);
...
Функция ПолучитьИдентификаторыСообщенийКФормированию(Количество = 0) Экспорт
    ...
    Запрос.Текст = СтрШаблон("ВЫБРАТЬ %1
                   ...
                   |    И инт_ТекущийСтатусВходящихСообщений.ДатаСледующейПопытки <= &ТекущийМомент
                   |;
                   |
                   |////////////////////////////////////////////////////////////////////////////////
                   |ВЫБРАТЬ %1
...
Функция РасчитатьДатуСледующейПопытки(ИдентификаторСообщения)
    ...
    Если НЕ ЗначениеЗаполнено(ИнформацияОСообщении.ПотокДанных) Тогда
        // Записи в очереди нет - рассчитать паузу по потоку невозможно. Возвращаем текущий момент.
        СообщениеОбОшибке = СтрШаблон("Сообщение %1 отсутствует в очереди входящих, дата следующей попытки рассчитана без учета паузы потока", ИдентификаторСообщения);
        ...
        Возврат инт_ОбщегоНазначения.ВремяБазы();
    КонецЕсли;

    Возврат Справочники.инт_ПотокиДанных.РасчитатьДатуСледующейПопыткиПоДате(инт_ОбщегоНазначения.ВремяБазы(),ИнформацияОСообщении.ПотокДанных);

КонецФункции

// Функция - Проверенный идентификатор сообщения
// Проверяет заполненность идентификатора сообщения и приводит его к типу УникальныйИдентификатор.
//
// Параметры:
//  ИдентификаторСообщения     -   УникальныйИдентификатор, Строка   - ИдентификаторСообщения
//
// Возвращаемое значение:
//   УникальныйИдентификатор - Идентификатор сообщения.
//
Функция ПроверенныйИдентификаторСообщения(Знач ИдентификаторСообщения)
    Если ТипЗнч(ИдентификаторСообщения) = Тип("Строка") И ЗначениеЗаполнено(ИдентификаторСообщения) Тогда
        ИдентификаторСообщения = Новый УникальныйИдентификатор(ИдентификаторСообщения);
    КонецЕсли;

    Если ТипЗнч(ИдентификаторСообщения) <> Тип("УникальныйИдентификатор") ИЛИ НЕ ЗначениеЗаполнено(ИдентификаторСообщения) Тогда
