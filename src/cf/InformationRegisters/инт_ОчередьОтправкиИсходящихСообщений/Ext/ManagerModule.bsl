...
Процедура СформироватьСообщение(Сообщение) Экспорт
	ДанныеИсходящегоСообщения = РегистрыСведений.инт_ОчередьИсходящихСообщений.ПолучитьДанныеОчередиПоИдентификатору(Сообщение.ИдентификаторСообщения, "СформированноеСообщение,ПотокДанных");

    Если НЕ ЗначениеЗаполнено(ДанныеИсходящегоСообщения.ПотокДанных) Тогда
        // Записи в очереди нет. Незаполненное сообщение сериализуется в null без ошибки,
        // поэтому без явной проверки подписчик получил бы пустое тело со статусом "Сформирован".
        ВызватьИсключение СтрШаблон("Сообщение <%1> отсутствует в очереди исходящих, сформировать его для подписчика <%2> нельзя.",
            Сообщение.ИдентификаторСообщения, Сообщение.Подписчик);
    КонецЕсли;

    СериализованноеСообщение = инт_ПодписчикиОбщий.СформироватьСообщение(ДанныеИсходящегоСообщения.СформированноеСообщение, Сообщение.Подписчик, Сообщение.ИдентификаторСообщения);
...
Процедура ОтправитьСообщение(Сообщение, ОтложенныеДействия) Экспорт
    ...
    // Получить ПотокДанных для статистики
    ...
    ПотокДанных = ?(ЗначениеЗаполнено(ДанныеИсходящегоСообщения.ПотокДанных), ДанныеИсходящегоСообщения.ПотокДанных, Справочники.инт_ПотокиДанных.ПустаяСсылка());
    
    ВремяНачала = ТекущаяУниверсальнаяДатаВМиллисекундах();

    НачатьТранзакцию();
    Попытка
        инт_ПодписчикиОбщий.ОтправитьСообщение(ДанныеСообщения, ОтложенныеДействия);
        ...
        РегистрыСведений.инт_СтатистикаОбработкиСообщений.ЗаписатьСтатистикуОтправки(Сообщение.ИдентификаторСообщения, ПотокДанных, ДлительностьМс, Истина, Сообщение.Подписчик);
    Исключение
        ОтменитьТранзакцию();
        ДлительностьМс = ТекущаяУниверсальнаяДатаВМиллисекундах() - ВремяНачала;
        РегистрыСведений.инт_СтатистикаОбработкиСообщений.ЗаписатьСтатистикуОтправки(Сообщение.ИдентификаторСообщения, ПотокДанных, ДлительностьМс, Ложь, Сообщение.Подписчик);
