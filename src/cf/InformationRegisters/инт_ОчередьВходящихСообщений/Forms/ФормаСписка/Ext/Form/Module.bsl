...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ОткрытьФорму("РегистрСведений.инт_ОчередьВходящихСообщений.ФормаЗаписи", СтруктураПараметров,,,,, ОповещениеОЗакрытии);
...
Процедура ВернутьСообщениеВОчередьЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		ВернутьСообщениеВОчередьНаСервере(ДополнительныеПараметры.ИдентификаторСообщения);
...
Процедура ИзменитьСтатусЗавершение(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт
	...
	Если ВыбранныйЭлемент = ПредопределенноеЗначение("Перечисление.инт_СтатусыВходящихСообщений.Новый") Тогда
		ПараметрыВозврата = Новый Структура("ИдентификаторСообщения",
			Элементы.Список.ТекущиеДанные.ИдентификаторСообщения);
		ПоказатьВопрос(Новый ОписаниеОповещения("ВернутьСообщениеВОчередьЗавершение", ЭтотОбъект, ПараметрыВозврата),
...
Процедура ПослеВводаТекстаОшибки(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		Сообщение.Текст = "Статус не был изменен";
		Сообщение.Поле = "ПолеСтатусСообщения";
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
