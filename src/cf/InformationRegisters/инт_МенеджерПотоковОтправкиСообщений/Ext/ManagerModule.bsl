...
Процедура ОбработатьОткатСтатусаСообщений(СоставПотока)
    ...
    Для Каждого Сообщение Из МассивСообщений Цикл
        ...
        // Если статус "ВПроцессеОтправки" значит фоновое задание по какой-то причине упало раньше,
        ...
        Если Статус = Перечисления.инт_СтатусыРассылкиИсходящихСообщений.ВПроцессеОтправки Тогда
            ТекстОшибки = "Фоновое задание отправки завершалось аварийно. Число попыток отправки исчерпано.";
            НовыйСтатус = РегистрыСведений.инт_ТекущийСтатусРассылкиСообщений.ВернутьСообщениеВОчередь(
                ИдентификаторСообщения, Подписчик, ТекстОшибки);
