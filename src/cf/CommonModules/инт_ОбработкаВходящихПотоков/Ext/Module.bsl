...
Процедура инт_ОбработкаВходящейОчереди() Экспорт
    ...
    Для Каждого Идентификатор Из МассивИдентификаторов Цикл
        // Ошибка обработки одного сообщения не должна останавливать обработку всей пачки.
        Попытка
            РегистрыСведений.инт_ОчередьВходящихСообщений.ОбработатьСообщениеПоИдентификатору(Идентификатор);
        Исключение
            СообщениеОбОшибке = СтрШаблон("При обработке входящего сообщения с идентификатором <%1> из очереди возникла ошибка.
            |
