...
//
Функция СоздатьКонсьюмер(Брокеры, ГруппаКонсьюмеров, МассивТопиков, Эндпоинт = Неопределено) Экспорт
...
Процедура ОтправитьВDLQ(Эндпоинт, DLQТопик, ОригинальноеТело, ОригинальныйТопик, Смещение, Раздел, ОписаниеОшибки) Экспорт
	...
	DLQСообщение.Вставить("failed_at", Формат(ТекущаяУниверсальнаяДата(), "ДФ=yyyy-MM-ddTHH:mm:ssZ"));
	...
	Попытка
		...
		инт_РаботаСКафка.ОтправитьСообщение(DLQТело, Брокеры, DLQТопик, , , , , Эндпоинт);
