...
Функция ПолноеПредставление() Экспорт
	...
	Версия = ИТК_ТуллкитКлиентСерверПовтИсп.Версия();
...
Функция ОграниченнаяВерсия() Экспорт
	...
	Результат = СтрЗаканчиваетсяНа(ИТК_ТуллкитКлиентСервер.Идентификатор(), "ADMIN");
	Возврат Результат;
...
Функция КартинкаИнструмента(Инструмент) Экспорт
	...
	Если Инструмент = ИТК_Перечисления.ИнструментПодпискиНаСобытия() Тогда
		
		Результат = БиблиотекаКартинок.ИТК_ПодпискаНаСобытие;
	...
	ИначеЕсли Инструмент = ИТК_Перечисления.ИнструментОткрытьОбъект() Тогда
