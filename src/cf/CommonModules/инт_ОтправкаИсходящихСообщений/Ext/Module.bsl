...
Процедура СоздатьПотокиОтправкиСообщенийПоТипу(ТипПодписчика)
    КоличествоПотоковКСозданию = РегистрыСведений.инт_МенеджерПотоковОтправкиСообщений.КоличествоПотоковКСозданию(ТипПодписчика);
...
Процедура СоздатьПотокОтправкиСообщений(ТипПодписчика)
    ...
    МассивСообщений = РегистрыСведений.инт_ТекущийСтатусРассылкиСообщений.ПолучитьСообщенияКОтправке(КоличествоСообщенийВПачке, ТипПодписчика);
    ...
    НачатьТранзакцию();
    Попытка
        РегистрыСведений.инт_ТекущийСтатусРассылкиСообщений.ЗаписатьСтатусСообщений(МассивСообщений, Перечисления.инт_СтатусыРассылкиИсходящихСообщений.ВПроцессеОтправки);
        МассивПараметров = Новый Массив;
        МассивПараметров.Добавить(МассивСообщений);
        Поток = ФоновыеЗадания.Выполнить("инт_ОтправкаИсходящихСообщенийСлужебный.ВыполнитьОбработкуСообщенийВПотоке", МассивПараметров, ,СтрШаблон("Поток отправки исходящих сообщений по Типу подписчика <%1>.",ТипПодписчика));
        РегистрыСведений.инт_МенеджерПотоковОтправкиСообщений.ЗарегистрироватьПоток(Поток.УникальныйИдентификатор, ТипПодписчика, МассивСообщений);
