...
Процедура ЦиклЧтенияСообщений(Компонента, СоответствиеТопикПодписка, МассивПодписок)
	...
	// Формируем массив ссылок подписок для проверки статуса drain/stop
	МассивПодписокСсылки = Новый Массив;
	Для Каждого СтрокаПодписки Из МассивПодписок Цикл
		Если МассивПодписокСсылки.Найти(СтрокаПодписки.Подписка) = Неопределено Тогда
			МассивПодписокСсылки.Добавить(СтрокаПодписки.Подписка);
		КонецЕсли;
	КонецЦикла;
	...
	Пока РазрешеноСлушать Цикл
		...
		Если НЕ РезультатЧтения Тогда
			...
			РазрешеноСлушать = ПроверитьАктивностьПодписок(МассивПодписок, МассивПодписокСсылки);
			Продолжить;
		КонецЕсли;
...
// Проверяет активность подписок и наличие запросов на остановку (drain/graceful stop).
//
// Параметры:
//  МассивПодписок       - Массив из Структура - Подписки консьюмера
//  МассивПодписокСсылки - Массив из ОпределяемыйТип.инт_Подписка - Ссылки подписок для проверки статуса
//
// Возвращаемое значение:
//  Булево - Истина, если разрешено продолжать слушать
//
Функция ПроверитьАктивностьПодписок(МассивПодписок, МассивПодписокСсылки)
	
	// 1. Проверяем глобальный флаг graceful stop
	Если Константы.инт_ОстановитьВсеКонсьюмеры.Получить() Тогда
		ЗаписьЖурналаРегистрации(
			"ПодсистемаИнтеграции.КонсьюмерОчередей",
			УровеньЖурналаРегистрации.Информация,
			,
			,
			"Остановка консьюмера: получен сигнал GracefulStop (константа)");
		Возврат Ложь;
	КонецЕсли;
	
	// 2. Проверяем запрос на остановку (Drained/GracefulStop) в регистре
	...
	Если РезультатПроверки.НужнаОстановка Тогда
		ЗаписьЖурналаРегистрации(
			...
			,
			,
			СтрШаблон("Остановка консьюмера: получен запрос '%1'", РезультатПроверки.Причина));
		Возврат Ложь;
	КонецЕсли;
	
	// 3. Проверяем активность подписок (исходная логика)
	Для Каждого СтрокаПодписки Из МассивПодписок Цикл
