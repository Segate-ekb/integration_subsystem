...
//     * НомерСтрокиОригинала - Число - номер строки в оригинальном коде (извлечён из маркера //@@N)
//
		...
		Для Индекс = ИндексПоПричине + 1 По СтрокиСтека.ВГраница() Цикл
			...
			Если СтрНачинаетсяС(ТриммированнаяСтрока, "{<") Тогда
				...
				Если ПозицияДвоеточия > 0 Тогда
					...
					// Извлекаем оригинальный номер строки из маркера //@@N
					Результат.НомерСтрокиОригинала = ИзвлечьНомерСтрокиИзМаркера(Результат.Описание);
				КонецЕсли;
	...
	Пока Индекс >= 0 Цикл
		...
		Если СтрНачинаетсяС(ТриммированнаяСтрока, "{(") Тогда
			...
			Если ПозицияДвоеточия > 0 Тогда
				Результат.ИсходнаяСтрока = СокрЛП(Сред(Строка, ПозицияДвоеточия + 2));
				// Извлекаем оригинальный номер строки из маркера //@@N
				Результат.НомерСтрокиОригинала = ИзвлечьНомерСтрокиИзМаркера(Результат.ИсходнаяСтрока);
			КонецЕсли;
...
#Область РеверсМаппингСтрок

// Извлекает номер оригинальной строки.
// Функция-заглушка после удаления инлайнера.
// 
// Параметры:
//   ТекстКода - Строка - текст кода
//   НомерСтроки - Число - номер строки (нумерация с 1)
//
// Возвращаемое значение:
//   Число - НомерСтроки (возвращается без изменений)
//
Функция ИзвлечьОригинальнуюСтроку(ТекстКода, НомерСтроки) Экспорт
	
	Возврат НомерСтроки;
	
КонецФункции

// Извлекает номер строки из маркера //@@N.
// Функция-заглушка после удаления инлайнера — маркеры больше не используются.
// 
// Параметры:
//   СтрокаКода - Строка - строка кода
//
// Возвращаемое значение:
//   Число - 0 (маркеры больше не используются)
//
Функция ИзвлечьНомерСтрокиИзМаркера(СтрокаКода) Экспорт
