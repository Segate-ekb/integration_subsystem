...
Функция ОтправитьЗапрос(Сессия, ПодготовленныйЗапрос, Настройки)
    ...
    ПодготовленныйОтвет.Тело = Ответ.ПолучитьТелоКакСтроку();
...
Функция ПараметрыПреобразованияJSONПоУмолчанию()
	...
	ПараметрыПреобразованияПоУмолчанию.Вставить("ИмяФункцииПреобразования", "ПреобразованиеПоУмолчаниюПриСериализацииJSON");
	ПараметрыПреобразованияПоУмолчанию.Вставить("МодульФункцииПреобразования", инт_КоннекторHTTP);
...
// Функция - Преобразование при сериализации JSON
// Функция вызываемая при сериализации JSON, здесь будут обрабатываться все значения которые не подлежат прямой сериализации.
// Параметры:
//  Свойство                 - Произвольный  - в параметр передается имя свойства, если выполняется запись структуры или соответствия
//  Значение                 -   Произвольный   -  в параметр передается исходное значение,
//  ДополнительныеПараметры     -   Произвольный   -  дополнительные параметры, которые указаны в вызове метода ЗаписатьJSON,
//  Отказ                     -  Булево - отказ от записи свойства.
// 
// Возвращаемое значение:
//  Произвольный -  Значение после перобразования
//
Функция ПреобразованиеПоУмолчаниюПриСериализацииJSON(Свойство, Значение, ДополнительныеПараметры, Отказ) Экспорт
    
    Если ТипЗнч(Значение) = Тип("УникальныйИдентификатор") Тогда
    	Возврат Строка(Значение);
    ИначеЕсли ТипЗнч(Значение) = Тип("СправочникСсылка.инт_Подписчики") Тогда
