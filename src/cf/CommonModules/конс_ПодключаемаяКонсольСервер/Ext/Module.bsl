...
Процедура ДобавитьПеременную(Форма, ИмяПеременной, ТипИлиЗначение = Неопределено, Описание = "") Экспорт
	...
	// Сохраняем описание и имя типа прямо в JSON для подсказки контекста
	ИмяТипа = ОпределитьИмяТипа(ТипИлиЗначение);
	Если ЗначениеЗаполнено(ИмяТипа) Тогда
		Переменная.Вставить("typeName", ИмяТипа);
	КонецЕсли;
	Если ЗначениеЗаполнено(Описание) Тогда
		Переменная.Вставить("description", Описание);
	КонецЕсли;
...
Процедура ОбновитьПодсказкуРедактора(Форма)
	...
	// Переменные (описание и тип хранятся прямо в JSON переменных)
	Переменные = ПрочитатьJSONПеременных(Форма);
	Для Каждого КлючЗначение Из Переменные Цикл
		...
		Если ТипЗнч(Данные) = Тип("Структура") Тогда
			ИмяТипа = "";
			Если Данные.Свойство("typeName", ИмяТипа) И ЗначениеЗаполнено(ИмяТипа) Тогда
				СтрокаПодсказки = СтрокаПодсказки + " - " + ИмяТипа;
			КонецЕсли;
			ОписаниеПерем = "";
			Если Данные.Свойство("description", ОписаниеПерем) И ЗначениеЗаполнено(ОписаниеПерем) Тогда
				СтрокаПодсказки = СтрокаПодсказки + " - " + ОписаниеПерем;
			КонецЕсли;
		КонецЕсли;
