...
Процедура ЗаполнитьСтекВызовов(СтекВызовов, ИнформацияОбОшибке)
	...
	Для НомерСтрокиСтекаВызовов = 1 По КоличествоСтрок - 1 Цикл
		...
		НомерСимвола = 1;
		ИТК_Строки.РазборПропуститьНаборСимволов(СтрокаСтекаВызовов, "{", НомерСимвола);
		ИмяМодуля = ИТК_Строки.РазборПрочитатьДоСимвола(СтрокаСтекаВызовов, "(", НомерСимвола);
		
		ИТК_Строки.РазборПропуститьНаборСимволов(СтрокаСтекаВызовов, "(", НомерСимвола);
		НомерСтроки = ИТК_Строки.РазборПрочитатьЦелоеЧисло(СтрокаСтекаВызовов, НомерСимвола);
		ИТК_Строки.РазборПропуститьНаборСимволов(СтрокаСтекаВызовов, ")}:", НомерСимвола);
		
		ИсходнаяСтрока = ИТК_Строки.РазборПрочитатьДоСимвола(СтрокаСтекаВызовов, "", НомерСимвола);
