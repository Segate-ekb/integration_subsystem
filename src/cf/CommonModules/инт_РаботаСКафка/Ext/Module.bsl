...
//
Процедура ОтправитьСообщение(ОтправляемоеСообщение, Брокеры, Топик, НомерПартиции = -1, ИдентификаторСообщения = "", Заголовки = "", ТипСжатия = Неопределено) Экспорт
...
//  ТипСжатия       - ПеречислениеСсылка.инт_ТипыСжатияKafka - Тип сжатия сообщений (опционально)
//
Процедура ОтправитьСообщенияПакетом(МассивСообщений, Брокеры, Топик, ТипСжатия = Неопределено) Экспорт
...
//   * ОписаниеОшибки - Строка - Текст ошибки
//
Функция УстановитьНастройкиТопика(Брокеры, ИмяТопика, Настройки) Экспорт
...
Функция ПолучитьЭкземплярКомпоненты()
	ПодключитьВнешнююКомпоненту("ОбщийМакет.инт_simple_kafka", "Integration", ТипВнешнейКомпоненты.Native, ТипПодключенияВнешнейКомпоненты.НеИзолированно);
...
Функция ПолучитьКаталогЛоговKafka()
	...
	Если ЗначениеЗаполнено(КаталогЛогов) Тогда
		...
		Возврат КаталогВременныхФайлов()+"simple\";
	КонецЕсли;
