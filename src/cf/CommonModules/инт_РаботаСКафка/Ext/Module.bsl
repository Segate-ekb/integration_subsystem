	...
	Попытка
		...
		Если МассивПартиций <> Неопределено Тогда
			Для Каждого Партиция Из МассивПартиций Цикл
				ДанныеПартиции = Новый Структура("НомерПартиции, Leader, HighWatermark, LowWatermark");
				ДанныеПартиции.НомерПартиции = Число(Партиция.Получить("id"));
				ДанныеПартиции.Leader = Партиция.Получить("leader");
				HighWM = Партиция.Получить("high_watermark");
				ДанныеПартиции.HighWatermark = ?(HighWM <> Неопределено, Число(HighWM), 0);
				LowWM = Партиция.Получить("low_watermark");
				ДанныеПартиции.LowWatermark = ?(LowWM <> Неопределено, Число(LowWM), 0);
...
// Получает отставание (lag) consumer group для топика.
// Использует Admin API метод ПолучитьОтставаниеКонсьюмера — возвращает
// committed_offset, high_watermark и lag для каждой партиции.
//
// Параметры:
//  Брокеры               - Строка - Адреса брокеров Kafka через запятую
...
//   * ОписаниеОшибки - Строка - Текст ошибки
//   * Оффсеты - Массив из Структура - Данные оффсетов:
//       ** НомерПартиции  - Число - Номер партиции
//       ** ТекущийОффсет  - Число - Зафиксированное смещение (committed_offset)
//       ** HighWatermark  - Число - High watermark (конец лога)
//       ** LowWatermark   - Число - Low watermark (начало доступных сообщений)
//       ** Лаг            - Число - Отставание (high_watermark - committed_offset)
//
Функция ПолучитьОффсетыКонсьюмера(Брокеры, ИмяГруппыКонсьюмеров, ИмяТопика, Эндпоинт = Неопределено) Экспорт
	...
	Попытка
		...
		// Admin API: ПолучитьОтставаниеКонсьюмера(Брокер, ИмяТопика, ГруппаКонсьюмеров, Таймаут)
		// Возвращает JSON с partitions: [{partition, low_watermark, high_watermark, committed_offset, lag}]
		РезультатJSON = Компонента.ПолучитьОтставаниеКонсьюмера(Брокеры, ИмяТопика, ИмяГруппыКонсьюмеров, 5000);
		...
		// Ответ: {"topic":"...", "consumer_group":"...", "total_lag":"...",
		//         "partitions":[{"partition":"0", "low_watermark":"0",
		//          "high_watermark":"1000", "committed_offset":"950", "lag":"50"}]}
		ДанныеОтставания = инт_КоннекторHTTP.JsonВОбъект(РезультатJSON);
		
		МассивПартиций = ДанныеОтставания.Получить("partitions");
		Если МассивПартиций <> Неопределено Тогда
			Для Каждого ДанныеПартиции Из МассивПартиций Цикл
				ДанныеОффсета = Новый Структура("НомерПартиции, ТекущийОффсет, HighWatermark, LowWatermark, Лаг");
				ДанныеОффсета.НомерПартиции = Число(ДанныеПартиции.Получить("partition"));
				
				CommittedOffset = ДанныеПартиции.Получить("committed_offset");
				Если CommittedOffset = "none" ИЛИ CommittedOffset = Неопределено Тогда
					ДанныеОффсета.ТекущийОффсет = 0;
				Иначе
					ДанныеОффсета.ТекущийОффсет = Число(CommittedOffset);
				КонецЕсли;
				
				ДанныеОффсета.HighWatermark = Число(ДанныеПартиции.Получить("high_watermark"));
				ДанныеОффсета.LowWatermark = Число(ДанныеПартиции.Получить("low_watermark"));
				ДанныеОффсета.Лаг = Число(ДанныеПартиции.Получить("lag"));
...
// Инициализирует консьюмер, подписывается на топик, фиксирует смещение и останавливается.
//
// Параметры:
//  Брокеры               - Строка - Адреса брокеров Kafka через запятую
//  ИмяГруппыКонсьюмеров  - Строка - Consumer group ID
...
Функция УстановитьОффсетКонсьюмера(Брокеры, ИмяГруппыКонсьюмеров, ИмяТопика, НомерПартиции, НовыйОффсет, Эндпоинт = Неопределено) Экспорт
	...
	Попытка
		...
		// Подписываемся + делаем poll для получения назначения партиций
		Компонента.Подписаться(ИмяТопика);
		Компонента.ПрочитатьСообщение();
		
		// Фиксируем новое смещение: ЗафиксироватьСмещение(Топик, Смещение, Партиция)
		Если Компонента.ЗафиксироватьСмещение(ИмяТопика, НовыйОффсет, НомерПартиции) Тогда
			Результат.Успех = Истина;
			ЗаписьЖурналаРегистрации("ПодсистемаИнтеграции.РаботаСКафка",
				УровеньЖурналаРегистрации.Информация, , ,
