////////////////////////////////////////////////////////////////////////////////
// инт_ГенераторОбработокПовтИсп
//
// Кэширование значений модуля генерации обработок (EPF).
// Повторное использование возвращаемых значений: В течение сеанса.
//
// Структура дискового кэша:
//   {КаталогВременныхФайлов}/инт_epf_cache/
//     {GUID объекта}/           ← подпапка по потоку/подписчику
//       {хеш_текста_v1}.epf     ← версия обработчика
//       {хеш_текста_v2}.epf     ← новая версия после редактирования
//
////////////////////////////////////////////////////////////////////////////////

#Область СлужебныйПрограммныйИнтерфейс

// Функция - Корневой каталог кэша EPF
// Возвращает путь к статичному каталогу файлового кэша EPF.
// Каталог создаётся при первом обращении и не меняется в течение сеанса.
//
// Возвращаемое значение:
//  Строка - Полный путь к корневому каталогу кэша (с разделителем на конце).
//
Функция КорневойКаталогКэша() Экспорт
	
	Каталог = КаталогВременныхФайлов() + "инт_epf_cache" + ПолучитьРазделительПути();
	
	КаталогФайл = Новый Файл(Каталог);
	Если Не КаталогФайл.Существует() Тогда
		СоздатьКаталог(Каталог);
	КонецЕсли;
	
	Возврат Каталог;
	
КонецФункции

// Функция - Путь к платформе
// Кэширует путь к каталогу платформы 1С для компиляции EPF.
// Вызывается один раз за сеанс, далее берётся из кэша.
//
// Возвращаемое значение:
//  Строка - Путь к каталогу с исполняемым файлом 1cv8.
//
Функция ПутьКПлатформе() Экспорт
	
	// Используем каталог текущего приложения.
	КаталогПриложения = КаталогПрограммы();
	
	Если Не ПустаяСтрока(КаталогПриложения) Тогда
		Возврат КаталогПриложения;
	КонецЕсли;
	
	// Запасной вариант — стандартные пути.
	СистемнаяИнформация = Новый СистемнаяИнформация;
	ЭтоLinux = СистемнаяИнформация.ТипПлатформы = ТипПлатформы.Linux_x86 
		ИЛИ СистемнаяИнформация.ТипПлатформы = ТипПлатформы.Linux_x86_64;
	
	Если ЭтоLinux Тогда
		Возврат "/opt/1cv8/x86_64/" + СистемнаяИнформация.ВерсияПриложения + "/";
	Иначе
		Возврат "C:\Program Files\1cv8\" + СистемнаяИнформация.ВерсияПриложения + "\bin\";
	КонецЕсли;
	
КонецФункции

// Функция - Путь EPF потока данных
// Возвращает путь к файлу EPF обработчика потока данных на диске.
// Если файл отсутствует — получает из БД или компилирует.
// Результат кэшируется в течение сеанса (ПовтИсп).
//
// Параметры:
//  ПотокДанных - СправочникСсылка.инт_ПотокиДанных - Ссылка на поток данных.
//
// Возвращаемое значение:
//  Строка - Полный путь к EPF-файлу обработчика. Пустая строка, если обработчик не задан.
//
Функция ПутьЕПФПотока(Знач ПотокДанных) Экспорт
	
	Если ПотокДанных.Пустая() Тогда
		Возврат "";
	КонецЕсли;
	
	ТекстОбработчика = ПотокДанных.ТекстОбработчика;
	Если Не ЗначениеЗаполнено(ТекстОбработчика) Тогда
		Возврат "";
	КонецЕсли;
	
	ХешТекста = инт_ГенераторОбработок.ХешТекстаОбработчика(ТекстОбработчика);
	ИдОбъекта = Строка(ПотокДанных.УникальныйИдентификатор());
	КаталогОбъекта = инт_ГенераторОбработок.КаталогКэшаОбъекта(ИдОбъекта);
	ПутьКФайлу = КаталогОбъекта + ХешТекста + ".epf";
	
	// 1. Проверяем наличие на диске.
	Файл = Новый Файл(ПутьКФайлу);
	Если Файл.Существует() Тогда
		ЗаписьЖурналаРегистрации("ПодсистемаИнтеграции.КэшEPF",
			УровеньЖурналаРегистрации.Примечание, , ПотокДанных,
			СтрШаблон("Кэш-попадание (диск): %1", ПутьКФайлу));
		Возврат ПутьКФайлу;
	КонецЕсли;
	
	// 2. Получаем скомпилированный обработчик из БД.
	ДвоичныеДанныеEPF = ПотокДанных.СкомпилированныйОбработчик.Получить();
	Если ДвоичныеДанныеEPF <> Неопределено Тогда
		ДвоичныеДанныеEPF.Записать(ПутьКФайлу);
		ЗаписьЖурналаРегистрации("ПодсистемаИнтеграции.КэшEPF",
			УровеньЖурналаРегистрации.Примечание, , ПотокДанных,
			СтрШаблон("Кэш-промах (диск). Записан из БД: %1", ПутьКФайлу));
		Возврат ПутьКФайлу;
	КонецЕсли;
	
	// 3. Компилируем заново из текста.
	ДвоичныеДанныеEPF = инт_ГенераторОбработок.СкомпилироватьОбработку(ТекстОбработчика);
	ДвоичныеДанныеEPF.Записать(ПутьКФайлу);
	ЗаписьЖурналаРегистрации("ПодсистемаИнтеграции.КэшEPF",
		УровеньЖурналаРегистрации.Предупреждение, , ПотокДанных,
		СтрШаблон("Кэш-промах (диск+БД). Скомпилирован заново: %1", ПутьКФайлу));
	
	Возврат ПутьКФайлу;
	
КонецФункции

#КонецОбласти
