...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Автозаполнение имени группы консьюмеров (такое же, как при инициализации консьюмера)
	ИмяГруппыКонсьюмеров = инт_КонсьюмерОчередейСлужебный.СформироватьИдентификаторГруппы("consumer");
...
Процедура ЗагрузитьОффсеты(Команда)
	...
	Если ПустаяСтрока(ИмяГруппыКонсьюмеров) Тогда
		ПоказатьПредупреждение(, "Не указано имя consumer group!");
		Возврат;
	КонецЕсли;
...
Процедура ПрименитьОффсеты(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения,
		СтрШаблон("Применить новые оффсеты для consumer group '%1' в топике '%2'?
			|
			|Внимание: консьюмер должен быть остановлен!",
			ИмяГруппыКонсьюмеров, Объект.ИмяТопика),
		РежимДиалогаВопрос.ДаНет);
...
Процедура ЗагрузитьОффсетыНаСервере()
	...
	Попытка
		РезультатОффсетов = инт_РаботаСКафка.ПолучитьОффсетыКонсьюмера(
			Брокеры, ИмяГруппыКонсьюмеров, Объект.ИмяТопика, Объект.Эндпоинт);
	Исключение
	...
	// Получаем информацию о партициях (для watermarks)
	Попытка
		РезультатПартиций = инт_РаботаСКафка.ПолучитьИнформациюОПартициях(
			Брокеры, Объект.ИмяТопика, Объект.Эндпоинт);
	Исключение
		Сообщить("Ошибка получения информации о партициях: " + ОписаниеОшибки());
		Возврат;
	КонецПопытки;
	
	ВотермаркиПоПартициям = Новый Соответствие;
	Если РезультатПартиций.Успех Тогда
		Для Каждого ДанныеПартиции Из РезультатПартиций.Партиции Цикл
			ВотермаркиПоПартициям.Вставить(ДанныеПартиции.НомерПартиции, ДанныеПартиции);
		КонецЦикла;
	КонецЕсли;
	...
	Для Каждого ДанныеОффсета Из РезультатОффсетов.Оффсеты Цикл
		...
		ДанныеВотермарка = ВотермаркиПоПартициям.Получить(ДанныеОффсета.НомерПартиции);
		Если ДанныеВотермарка <> Неопределено Тогда
			НоваяСтрока.HighWatermark = ДанныеВотермарка.HighWatermark;
			НоваяСтрока.Лаг = ДанныеВотермарка.HighWatermark - ДанныеОффсета.ТекущийОффсет;
		КонецЕсли;
	КонецЦикла;
