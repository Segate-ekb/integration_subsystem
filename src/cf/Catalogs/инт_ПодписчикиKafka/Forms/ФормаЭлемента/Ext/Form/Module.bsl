...
Процедура ОтправитьСообщение(Команда)
	...
	ПараметрыОповещения = Новый Структура("Эндпоинт, ИмяТопика, ТипСжатия", Объект.Эндпоинт, Объект.ИмяТопика, Объект.ТипСжатия);
...
Процедура ОтправитьСообщениеЗавершение(ТекстСообщения, ДополнительныеПараметры) Экспорт
	...
	Если РезультатОтправки.Успех Тогда
		ПоказатьПредупреждение(, СтрШаблон("Сообщение успешно отправлено в топик '%1'", ДополнительныеПараметры.ИмяТопика));
	Иначе
...
Функция ОтправитьСообщениеНаСервере(Эндпоинт, ИмяТопика, ТекстСообщения, ТипСжатия)
	...
	Попытка
		...
		Ключ = Строка(Новый УникальныйИдентификатор()); // Генерируем случайный ключ
		
		инт_РаботаСКафка.ОтправитьСообщение(ТекстСообщения, Брокеры, ИмяТопика, ,,,ТипСжатия);
