...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	ТипАвторизацииСтр = Строка(Объект.ТипАвторизации);
	Если ПустаяСтрока(ТипАвторизацииСтр) Тогда
		ТипАвторизацииСтр = "Анонимный";
	КонецЕсли;
	
	Адрес = Объект.АдресРесурса;
	Если СтрДлина(Адрес) > 50 Тогда
		Адрес = Лев(Адрес, 50) + "...";
	КонецЕсли;
	
	Объект.Наименование = СтрШаблон("[%1] %2", ТипАвторизацииСтр, Адрес);
...
Функция ПрочитатьИзБезопасногоХранилища(Владелец, Ключ)
	...
	Результат = инт_РаботаСБезопаснымХранилищем.ПрочитатьДанныеИзБезопасногоХранилища(Владелец, Ключ);
...
Процедура ЗаписатьВБезопасноеХранилище(Владелец, Ключ, Значение)
	...
	УстановитьПривилегированныйРежим(Истина); // BSLLS:SetPrivilegedMode-off
	инт_РаботаСБезопаснымХранилищем.ЗаписатьДанныеВБезопасноеХранилище(Владелец, Значение, Ключ);
	УстановитьПривилегированныйРежим(Ложь);
