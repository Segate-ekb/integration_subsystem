...
Функция ПолучитьОбработчикПоУмолчанию(КодСостояния)
	Обработчик = "";
	// Здесь через ИНачеЕсли можно определить базовое поведение для разных кодов состояний. Если их будет много - вынести в регистр
	Если Не (КодСостояния >= 200 И КодСостояния < 300) Тогда
