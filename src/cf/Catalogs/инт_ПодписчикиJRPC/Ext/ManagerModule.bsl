    ...
    Если СтрокаПостПроцессинга <> Неопределено Тогда
        ...
        Если Файл.Существует() Тогда
            ЗаписьЖурналаРегистрации("ПодсистемаИнтеграции.КэшEPF",
                ...
                СтрШаблон("JRPC: Кэш-попадание (диск), код %1: %2", КодСостояния, ПутьКФайлу));
            Возврат ПутьКФайлу;
        КонецЕсли;
        ...
        Если ДвоичныеДанные <> Неопределено Тогда
            ...
            ЗаписьЖурналаРегистрации("ПодсистемаИнтеграции.КэшEPF",
                УровеньЖурналаРегистрации.Примечание, , Подписчик,
                СтрШаблон("JRPC: Кэш-промах (диск), записан из БД, код %1: %2", КодСостояния, ПутьКФайлу));
            Возврат ПутьКФайлу;
        КонецЕсли;
        ...
        ЗаписьЖурналаРегистрации("ПодсистемаИнтеграции.КэшEPF",
            ...
            СтрШаблон("JRPC: Кэш-промах (диск+БД), скомпилирован заново, код %1: %2", КодСостояния, ПутьКФайлу));
        Возврат ПутьКФайлу;
    Иначе
