...
Функция ПолучитьОбработчикПоКодуСостояния(КодСостояния, Подписчик)
    ...
    Если СтрокаПостПроцессинга <> Неопределено Тогда
        ...
        Возврат инт_ГенераторОбработок.РазрешитьEPF(
            СтрокаПостПроцессинга.Обработчик,
            ДвоичныеДанныеИзБД, ДопПеременные);
    Иначе
...
Процедура Исполнить(ДвоичныеДанныеEPF, Ответ, ИсходныеДанные, РазрешитьФиксациюИзменений = Ложь)
    
    Если ДвоичныеДанныеEPF = Неопределено Тогда
        Возврат;
    КонецЕсли;
    
    инт_ГенераторОбработок.ИсполнитьОбработкуПодписчика(ДвоичныеДанныеEPF, Ответ, ИсходныеДанные, РазрешитьФиксациюИзменений);
