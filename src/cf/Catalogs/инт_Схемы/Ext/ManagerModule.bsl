...
//
// Параметры:
//  Ссылка     - СправочникСсылка.инт_Схемы - Ссылка на схему
// 
// Возвращаемое значение:
//  Строка - Строка Json. Схема полученая по ссылке. 
//
Функция ПолучитьТекстJsonДляВнешнейСхемы(Схема) Экспорт
    ...
    Если Не (Ответ.КодСостояния >= 200 И Ответ.КодСостояния < 300)  Тогда
        ...
        | КодСостояния: %2
        | Ответ: %3", Схема.СсылкаНаСхему, Ответ.КодСостояния, ПолучитьСтрокуИзДвоичныхДанных(Ответ.Тело));
