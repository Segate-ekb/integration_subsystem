...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
    ...
    // Загружаем сохранённый режим отображения результата
    РежимОтображенияРезультата = ЗагрузитьНастройкиРежимаОтображения();
...
&НаКлиенте
Процедура РежимОтображенияРезультатаПриИзменении(Элемент)
	СохранитьНастройкиРежимаОтображения(РежимОтображенияРезультата);
КонецПроцедуры
...
Процедура РедакторКодаПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	ЗаполнитьСписокРеквизитовДляОтображения();
...
Функция ПроверкаПрошлаУспешно()
	Успех = Истина; 
	ФильтрПеременных = ПолучитьФильтрПеременныхИзСпискаРеквизитов();
	ДанныеКода = конс_ПодключаемаяКонсольКлиент.ПолучитьКодДляВнешнегоВыполнения(ЭтаФорма, РежимОтображенияРезультата, , ФильтрПеременных);
	// на время проверки заменим код обработчика на подготовленный;
	ИсходныйТекстОбработчика = Объект.ТекстОбработчика;
	Объект.ТекстОбработчика = ДанныеКода.Код;
	...
	// вернем все на место	
	объект.ТекстОбработчика = ИсходныйТекстОбработчика; 
	Возврат Успех;
КонецФункции
...
Процедура ДобавитьПеременные()
	...
	конс_ПодключаемаяКонсольСервер.ДобавитьПеременную(ЭтотОбъект, "Результат", Тип("Соответствие"));
	конс_ПодключаемаяКонсольСервер.ДобавитьПеременную(ЭтотОбъект, "ИсходныеДанные", ТипЗнч(ПримерСсылка));
	
КонецПроцедуры

// Преобразует список значений формы в массив имён переменных для фильтрации.
//
// Возвращаемое значение:
//   Массив из Строка - имена переменных из списка, или Неопределено если список пуст
//
&НаКлиенте
Функция ПолучитьФильтрПеременныхИзСпискаРеквизитов()
	
	Если СписокРеквизитовДляОтображенияРезультата.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	МассивИмен = Новый Массив;
	Для Каждого ЭлементСписка Из СписокРеквизитовДляОтображенияРезультата Цикл
		Если ЭлементСписка.Пометка Тогда
			МассивИмен.Добавить(ЭлементСписка.Значение);
		КонецЕсли;
	КонецЦикла;
...
// Заполняет список переменных для отображения на основе кода в редакторе.
//
&НаКлиенте
Процедура ЗаполнитьСписокРеквизитовДляОтображения()
	
	МассивПеременных = конс_ПодключаемаяКонсольКлиент.ПолучитьПеременныеИзКода(ЭтаФорма);
	
	// Запоминаем текущие отмеченные элементы
	ОтмеченныеРанее = Новый Соответствие;
	Для Каждого ЭлементСписка Из СписокРеквизитовДляОтображенияРезультата Цикл
		ОтмеченныеРанее.Вставить(ЭлементСписка.Значение, ЭлементСписка.Пометка);
	КонецЦикла;
	
	СписокРеквизитовДляОтображенияРезультата.Очистить();
	
	Для Каждого ИмяПеременной Из МассивПеременных Цикл
		// Если переменная была отмечена ранее - сохраняем её отметку
		Пометка = ОтмеченныеРанее.Получить(ИмяПеременной);
		Если Пометка = Неопределено Тогда
			Пометка = Истина; // Новые переменные отмечаем по умолчанию
		КонецЕсли;
		СписокРеквизитовДляОтображенияРезультата.Добавить(ИмяПеременной, ИмяПеременной, Пометка);
	КонецЦикла;
	
КонецПроцедуры

// Сохраняет режим отображения результата в общие настройки.
//
// Параметры:
//   РежимОтображения - ПеречислениеСсылка.конс_РежимПоказаПеременных - режим для сохранения
//
&НаСервереБезКонтекста
Процедура СохранитьНастройкиРежимаОтображения(Знач РежимОтображения)
	
	ХранилищеОбщихНастроек.Сохранить(
		"инт_ПотокиДанных_ФормаЭлемента", 
		"РежимОтображенияРезультата", 
		РежимОтображения);
	
КонецПроцедуры

// Загружает режим отображения результата из общих настроек.
//
// Возвращаемое значение:
//   ПеречислениеСсылка.конс_РежимПоказаПеременных - сохранённый режим или значение по умолчанию
//
&НаСервереБезКонтекста
Функция ЗагрузитьНастройкиРежимаОтображения()
	
	РежимОтображения = ХранилищеОбщихНастроек.Загрузить(
		"инт_ПотокиДанных_ФормаЭлемента", 
		"РежимОтображенияРезультата");
	
	Если Не ЗначениеЗаполнено(РежимОтображения) Тогда
		РежимОтображения = ПредопределенноеЗначение("Перечисление.конс_РежимПоказаПеременных.ВТабло");
	КонецЕсли;
	
	Возврат РежимОтображения;
