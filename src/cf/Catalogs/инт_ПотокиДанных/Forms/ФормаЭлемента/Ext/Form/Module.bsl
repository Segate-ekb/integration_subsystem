...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
    ...
    Если ЗначениеЗаполнено(ТекущийОбъект.ТекстОбработчика) Тогда
        ...
        Если ДвоичныеДанныеEPF <> Неопределено Тогда
            ...
            // Записываем EPF в дисковый кэш по GUID потока для кросс-сеансового использования.
            Если Не ТекущийОбъект.Ссылка.Пустая() Тогда
                ИдОбъекта = Строка(ТекущийОбъект.Ссылка.УникальныйИдентификатор());
                инт_ГенераторОбработок.ОбеспечитьEPFНаДиске(ДвоичныеДанныеEPF, ИдОбъекта, ТекущийХеш);
            КонецЕсли;
        КонецЕсли;
    КонецЕсли;
КонецПроцедуры
