...
Функция СформироватьСообщениеПоПотоку(ИсходныеДанные, ПотокДанных) Экспорт
    ...
    Попытка
        ИмяОбработки = инт_ГенераторОбработокПовтИсп.ИмяОбработкиПотока(ПотокДанных);
        СформированноеСообщение = Исполнить(ИмяОбработки, ИсходныеДанные);
    Исключение
...
Процедура ОбработатьВходящееСообщениеПоПотоку(СоответствиеСообщения, ПотокДанных, РазрешитьФиксациюИзменений = Истина) Экспорт
    ...
    Попытка
        ИмяОбработки = инт_ГенераторОбработокПовтИсп.ИмяОбработкиПотока(ПотокДанных);
        Исполнить(ИмяОбработки, СоответствиеСообщения, РазрешитьФиксациюИзменений);
    Исключение
...
Процедура ВалидироватьСообщениеПоПотоку(Сообщение, ПотокДанных) Экспорт
	Если Не ПотокДанных.Валидация Тогда
		...
		Возврат;
	КонецЕсли;
		МассивОшибок = инт_ВалидаторПакетов.Валидировать(Сообщение, ПотокДанных.ИмяСхемыПакета, ПотокДанных.СхемаДанных.ТекстСхемыJson);
...
// Функция - Исходные данные для отладчика
//	Готовит значение переменной ИсходныеДанные отладчика обработчика входящего потока.
//
// Параметры:
//  ТестовыеДанные	 -  Строка  -  JSON входящего сообщения из карточки потока.
//
// Возвращаемое значение:
//  Соответствие, Массив - разобранные тестовые данные;
//  Тип - Тип("Соответствие"), если тестовые данные не заполнены или не разбираются: отладчику известен только тип.
//
Функция ИсходныеДанныеДляОтладчика(ТестовыеДанные) Экспорт

	Если ЗначениеЗаполнено(ТестовыеДанные) Тогда

		Попытка
			РазобранныеДанные = инт_КоннекторHTTP.JsonВОбъект(ТестовыеДанные);
		Исключение
			// Об ошибке разбора пользователь узнает при проверке по примеру, в отладчик передаем только тип
			РазобранныеДанные = Неопределено;
		КонецПопытки;

		Если ТипЗнч(РазобранныеДанные) = Тип("Соответствие") Или ТипЗнч(РазобранныеДанные) = Тип("Массив") Тогда
			Возврат РазобранныеДанные;
		КонецЕсли;
...
Функция Исполнить(ИмяИлиДвоичныеДанныеEPF, ИсходныеДанные, РазрешитьФиксациюИзменений = Ложь)
    
    Возврат инт_ГенераторОбработок.ИсполнитьОбработку(ИмяИлиДвоичныеДанныеEPF, ИсходныеДанные, РазрешитьФиксациюИзменений);
