...
Функция СформироватьСообщениеПоПотоку(ИсходныеДанные, ПотокДанных) Экспорт
    ...
    Попытка
        ИмяОбработки = инт_ГенераторОбработок.ИмяОбработкиПотока(ПотокДанных);
...
Функция Исполнить(ИмяОбработки, ИсходныеДанные, РазрешитьФиксациюИзменений = Ложь)
    
    Возврат инт_ГенераторОбработок.ИсполнитьОбработку(ИмяОбработки, ИсходныеДанные, РазрешитьФиксациюИзменений);
