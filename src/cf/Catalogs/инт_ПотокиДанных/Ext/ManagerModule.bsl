...
//
// Описание:
//  Модуль менеджера справочника "Потоки данных".
//  Управляет формированием и обработкой сообщений по потокам данных.
//
// Основные возможности:
//  - Формирование исходящих сообщений через выполнение обработчика потока
//  - Обработка входящих сообщений через выполнение обработчика потока
//  - Валидация сообщений по OpenAPI схеме
//  - Получение списка подписчиков потока
//  - Управление настройками отслеживания изменений для объектов
//
// Поток данных:
//  Именованный канал обмена сообщениями с определенной логикой обработки.
//  Каждый поток имеет направление (исходящий/входящий) и обработчик - программный код,
//  который преобразует данные в сообщение (для исходящих) или обрабатывает сообщение (для входящих).
//
////////////////////////////////////////////////////////////////////////////////
...
//  ПотокДанных    - СправочникСсылка.инт_ПотокиДанных  - Поток данных для формирования сообщения
// 
// Возвращаемое значение:
//  Соответствие - Подготовленное для сериализации соответствие (JSON-подобная структура)
//
// Описание:
//  Формирует сообщение, выполняя обработчик потока данных для указанных исходных данных.
//  Обработчик потока - это программный код, который преобразует исходные данные в структуру сообщения.
//  
//  Процесс формирования:
//  1. Проверка, что поток является исходящим
//  2. Выполнение обработчика потока
//  3. Валидация сформированного сообщения (если включена в настройках потока)
//
//  При возникновении ошибки записывает информацию в журнал регистрации и генерирует исключение.
//
...
// Функция - Получить подписчиков по потоку
//
// Параметры:
//  ПотокДанных - СправочникСсылка.инт_ПотокиДанных - Поток данных
//
// Возвращаемое значение:
//  Массив из СправочникСсылка.инт_Подписчики - Массив ссылок на подписчиков потока
//
// Описание:
//  Возвращает список всех подписчиков, которые настроены для получения сообщений из указанного потока.
//  Используется при рассылке исходящих сообщений.
//
