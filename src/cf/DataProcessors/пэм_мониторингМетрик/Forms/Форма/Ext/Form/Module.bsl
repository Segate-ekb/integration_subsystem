...
Функция СформироватьОтчет()
	...
	Результат.Добавить("");
	Результат.Добавить("Дата формирования: " + Формат(инт_ОбщегоНазначения.ВремяБазы(), "ДЛФ=DDT"));
...
Процедура ДобавитьСекциюКлючевыеПоказатели(Результат)
	...
	ГраницыПериода = ГраницыПериодаОтчета();
	ДатаНачала = ГраницыПериода.ДатаНачала;
	ДатаОкончания = ГраницыПериода.ДатаОкончания;
...
Процедура ДобавитьСекциюСтатистикаПоПотокам(Результат)
	...
	Запрос.УстановитьПараметр("ДатаНачала", УниверсальноеВремяГраницыПериода(ГраницыПериода.ДатаНачала));
	Запрос.УстановитьПараметр("ДатаОкончания", УниверсальноеВремяГраницыПериода(ГраницыПериода.ДатаОкончания));
...
// Возвращает границы периода отчёта во времени базы - том же, в котором подсистема хранит
// моменты, поэтому заданный человеком период сравнивается с ними без преобразования.
// Незаполненное окончание периода означает конец текущего дня по времени базы.
//
// Возвращаемое значение:
//  Структура - Границы периода отчёта:
//   * ДатаНачала    - Дата - Начало периода во времени базы
//   * ДатаОкончания - Дата - Окончание периода во времени базы
//
&НаСервере
Функция ГраницыПериодаОтчета()

	ОкончаниеПериода = ?(ЗначениеЗаполнено(Период.ДатаОкончания),
		КонецДня(Период.ДатаОкончания), КонецДня(инт_ОбщегоНазначения.ВремяБазы()));

	Возврат Новый Структура("ДатаНачала, ДатаОкончания", Период.ДатаНачала, ОкончаниеПериода);

КонецФункции

// Возвращает границу периода в универсальном времени. Период регистра статистики обработки
// сообщений задан требованием ТР-Ф-018 в универсальном времени и настоящим требованием
// не меняется, поэтому граница приводится к нему из времени базы.
//
// Параметры:
//  ГраницаПериода - Дата - Граница периода отчёта во времени базы
//
// Возвращаемое значение:
//  Дата - Граница периода в универсальном времени
//
&НаСервереБезКонтекста
Функция УниверсальноеВремяГраницыПериода(ГраницаПериода)
	
	Возврат ?(ЗначениеЗаполнено(ГраницаПериода),
		УниверсальноеВремя(ГраницаПериода, инт_ОбщегоНазначения.ЧасовойПоясВремениБазы()),
		ГраницаПериода);
...
Процедура ДобавитьСекциюОшибки(Результат)
	...
	Запрос.УстановитьПараметр("ДатаНачала", ГраницыПериода.ДатаНачала);
	Запрос.УстановитьПараметр("ДатаОкончания", ГраницыПериода.ДатаОкончания);
