...
// Возвращает границы периода отчёта в универсальном времени. Периоды регистров хранятся
// в универсальном времени, а период отчёта человек задаёт в местном, поэтому сравнивать
// их напрямую нельзя. Незаполненное окончание периода означает конец текущего дня.
//
// Возвращаемое значение:
//  Структура - Границы периода отчёта:
//   * ДатаНачала    - Дата - Начало периода в универсальном времени
//   * ДатаОкончания - Дата - Окончание периода в универсальном времени
//
&НаСервере
Функция ГраницыПериодаОтчета()

	ОкончаниеПериода = ?(ЗначениеЗаполнено(Период.ДатаОкончания),
		КонецДня(Период.ДатаОкончания), КонецДня(ТекущаяДатаСеанса()));

	Возврат Новый Структура("ДатаНачала, ДатаОкончания",
		УниверсальноеВремяГраницыПериода(Период.ДатаНачала),
		УниверсальноеВремяГраницыПериода(ОкончаниеПериода));

КонецФункции

// Возвращает границу периода в универсальном времени: периоды регистров хранятся
// в универсальном времени, а поле формы - в местном.
//
// Параметры:
//  ГраницаПериода - Дата - Граница периода отчёта в местном времени сеанса.
//
// Возвращаемое значение:
...
Функция УниверсальноеВремяГраницыПериода(ГраницаПериода)
	...
	Возврат ?(ЗначениеЗаполнено(ГраницаПериода), УниверсальноеВремя(ГраницаПериода), ГраницаПериода);
...
Процедура ДобавитьСекциюОшибки(Результат)
	...
	Пока Выборка.Следующий() Цикл
		...
		Результат.Добавить("[" + НомерОшибки + "] " + Формат(МестноеВремя(Выборка.Период), "ДЛФ=DDT"));
...
Процедура ДобавитьСекциюМетрикиPrometheus(Результат)
	...
	Пока Выборка.Следующий() Цикл
		...
		Результат.Добавить("  • Расчет: " + Формат(МестноеВремя(Выборка.ДатаРасчета), "ДЛФ=DT"));
		Результат.Добавить("");
	КонецЦикла;
