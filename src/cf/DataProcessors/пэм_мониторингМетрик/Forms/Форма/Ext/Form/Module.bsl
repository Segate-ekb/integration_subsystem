...
Процедура ДобавитьСекциюСтатистикаПоПотокам(Результат)
	...
	ДатаНачала = УниверсальноеВремяГраницыПериода(Период.ДатаНачала);
	ДатаОкончания = УниверсальноеВремяГраницыПериода(ОкончаниеПериода);
...
// Возвращает границу периода в универсальном времени: период регистра статистики
// обработки сообщений хранится в универсальном времени, а поле формы - в местном.
//
// Параметры:
