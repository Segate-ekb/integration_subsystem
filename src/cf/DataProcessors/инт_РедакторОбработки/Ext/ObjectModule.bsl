////////////////////////////////////////////////////////////////////////////////
// инт_РедакторОбработки
//
// Редактор бинарного формата EPF.
// Позволяет распаковывать, модифицировать и запаковывать внутренние файлы обработки
// без вызова Конфигуратора.
//
// Реализовано на основании разработки https://infostart.ru/1c/tools/1691321/ by ZhokhovM
// Адаптировано из YAXUnit (https://github.com/bia-technologies/yaxunit),
...
// 
// Возвращаемое значение:
//  ДвоичныеДанные - Собранная обработка
...
Функция ПрочитатьКонтейнер(Поток, ПараметрыЗаписи, Распаковывать)
	...
	Оглавление = ДанныеОглавления(ПотокОглавления, ПараметрыЗаписи);
...
Функция ПотокДокумента(ВходнойПоток, ПараметрыЗаписи, АдресБлока)
	...
	Пока СледующийБлок < МаксимальноеЦелоеЧисло Цикл
		...
		Если ЭтоПервыйБлок Тогда
			...
			Запись = Новый ЗаписьДанных(ПотокЗаписи);
			ЭтоПервыйБлок = Ложь;
		КонецЕсли;
...
Функция ЗарезервироватьМестоОглавления(Поток, ПараметрыЗаписи, РазмерОглавления, ВыравниватьОглавление)
	...
	Поток.Записать(Новый БуферДвоичныхДанных(РезервируемыйРазмер, ПорядокБайтов.LittleEndian), 0, РезервируемыйРазмер);
...
Функция ДатаЧислом(ДатаВремя)
	...
	Множитель = 10000; // Время в 100-микросекундных интервалах с начала нашей эры (01.01.0001 00:00:00)
...
Функция СжатьДвоичныеДанные(ДвоичныеДанные)
	...
	// Локальное отличие от первоисточника YAXUnit: временный файл удаляется на всех путях,
	// а неудача удаления не подменяет исходную ошибку сжатия (ТР-НФ-052).
	...
	Исключение
		УдалитьВременныйФайлНеПодменяяОшибку(ИмяВременногоФайла);
		ВызватьИсключение;
	КонецПопытки;
...
// Удаляет временный файл, не подменяя исходную ошибку.
// Локальное отличие от первоисточника YAXUnit (ТР-НФ-052).
//
// Параметры:
//  ИмяВременногоФайла - Строка - Полное имя удаляемого файла.
//
Процедура УдалитьВременныйФайлНеПодменяяОшибку(Знач ИмяВременногоФайла)

	Попытка
		УдалитьФайлы(ИмяВременногоФайла); // BSLLS:UsingSynchronousCalls-off
	Исключение
		// Ошибка удаления не должна закрыть собой ошибку сжатия: до вызывающего кода
		// доходит исходное исключение.
		ЗаписьЖурналаРегистрации("ПодсистемаИнтеграции.ГенераторОбработок.ВременныйФайл",
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
			СтрШаблон("Не удалён временный файл сборки внешней обработки: %1", ИмяВременногоФайла));
	КонецПопытки;
