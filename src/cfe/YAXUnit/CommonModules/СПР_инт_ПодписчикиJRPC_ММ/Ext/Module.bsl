...
Процедура ИсполняемыеСценарии() Экспорт
	...
	ЮТТесты.УдалениеТестовыхДанных()
		...
		.ДобавитьТестовыйНабор("ПостОбработкаJRPC").ВТранзакции(Ложь).УдалениеТестовыхДанных()
			.ДобавитьТест("ПостОбработкаJRPC_ОбработчикТабличнойЧастиИсполняется")
			.ДобавитьТест("ПостОбработкаJRPC_ОбработчикПоУмолчаниюСообщаетКодИТело")
		.ДобавитьТестовыйНабор("КешированиеПостОбработкиJRPC").ВТранзакции(Ложь).УдалениеТестовыхДанных()
			.ДобавитьТест("КешированиеJRPC_ЗаписьПодписчикаЗаполняетСохраненнуюОбработку")
		.ДобавитьТестовыйНабор("ОтправкаСообщенияJRPC")
			.ДобавитьТест("ТестJRPCОтправкаБезЗаписиВОчередиОтклоняется");
...
// {{{ Набор: ПостОбработкаJRPC

// Обработчик из табличной части исполняется и меняет ответ так, как задано его текстом.
Процедура ПостОбработкаJRPC_ОбработчикТабличнойЧастиИсполняется() Экспорт
	
	// Arrange
	Подписчик = ГенераторТестовыхДанных.ПодписчикСПостОбработкой(
		ГенераторТестовыхДанных.ПодписчикJRPC(),
		200,
		ГенераторТестовыхДанных.ТекстПостОбработкиСМеткой("jrpc"));
	Ответ = ГенераторТестовыхДанных.ОтветПодписчика(200);
	
	// Act
	Справочники.инт_ПодписчикиJRPC.ВыполнитьПостПроцессинг(Подписчик, Ответ);
	
	// Assert
	ЮТест.ОжидаетЧто(Ответ, "Ответ после пост-обработки").Свойство("метка").Равно("jrpc");
	
КонецПроцедуры

// Обработчик по умолчанию сообщает и код состояния, и тело ответа: по одному телу
// причину отказа не восстановить.
Процедура ПостОбработкаJRPC_ОбработчикПоУмолчаниюСообщаетКодИТело() Экспорт
	
	// Arrange
	Подписчик = ГенераторТестовыхДанных.ПодписчикJRPC();
...
// Обработка собирается при записи подписчика: у справочника есть модуль объекта,
// вызывающий общую сборку.
Процедура КешированиеJRPC_ЗаписьПодписчикаЗаполняетСохраненнуюОбработку() Экспорт
	
	// Arrange + Act
	Подписчик = ГенераторТестовыхДанных.ПодписчикСПостОбработкой(
		ГенераторТестовыхДанных.ПодписчикJRPC(),
		200,
		ГенераторТестовыхДанных.ТекстПостОбработкиСМеткой("запись"));
...
// {{{ Набор: ОтправкаСообщенияJRPC

// Тест: без записи в очереди исходные данные не уйдут в постобработку как штатные
Процедура ТестJRPCОтправкаБезЗаписиВОчередиОтклоняется() Экспорт
	
	// Arrange
	Сообщение = Новый Структура("ИдентификаторСообщения, Подписчик, СформированноеСообщение",
		Новый УникальныйИдентификатор, ГенераторТестовыхДанных.ПодписчикJRPC(), "{}");
	
	// Act + Assert
	ЮТест.ОжидаетЧто(Справочники.инт_ПодписчикиJRPC)
