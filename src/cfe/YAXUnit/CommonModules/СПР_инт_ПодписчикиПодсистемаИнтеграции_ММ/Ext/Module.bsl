...
Процедура ИсполняемыеСценарии() Экспорт
	...
	ЮТТесты.УдалениеТестовыхДанных()
		...
		.ДобавитьТестовыйНабор("ПостОбработкаПодсистемаИнтеграции").ВТранзакции(Ложь).УдалениеТестовыхДанных()
			.ДобавитьТест("ПостОбработкаПодсистемаИнтеграции_ОбработчикТабличнойЧастиИсполняется")
			.ДобавитьТест("ПостОбработкаПодсистемаИнтеграции_ОбработчикПоУмолчаниюСообщаетКодИТело")
		.ДобавитьТестовыйНабор("КешированиеПостОбработкиПодсистемаИнтеграции").ВТранзакции(Ложь).УдалениеТестовыхДанных()
			.ДобавитьТест("КешированиеПодсистемаИнтеграции_ЗаписьПодписчикаЗаполняетСохраненнуюОбработку");
...
// {{{ Набор: ПостОбработкаПодсистемаИнтеграции

// Обработчик из табличной части исполняется и меняет ответ так, как задано его текстом.
Процедура ПостОбработкаПодсистемаИнтеграции_ОбработчикТабличнойЧастиИсполняется() Экспорт
	
	// Arrange
	Подписчик = ГенераторТестовыхДанных.ПодписчикСПостОбработкой(
		ГенераторТестовыхДанных.ПодписчикПодсистемаИнтеграции(),
		200,
		ГенераторТестовыхДанных.ТекстПостОбработкиСМеткой("подсистема интеграции"));
	Ответ = ГенераторТестовыхДанных.ОтветПодписчика(200);
	
	// Act
	Справочники.инт_ПодписчикиПодсистемаИнтеграции.ВыполнитьПостПроцессинг(Подписчик, Ответ);
	
	// Assert
	ЮТест.ОжидаетЧто(Ответ, "Ответ после пост-обработки").Свойство("метка").Равно("подсистема интеграции");
	
КонецПроцедуры

// Обработчик по умолчанию сообщает и код состояния, и тело ответа: по одному телу
// причину отказа не восстановить.
Процедура ПостОбработкаПодсистемаИнтеграции_ОбработчикПоУмолчаниюСообщаетКодИТело() Экспорт
	
	// Arrange
	Подписчик = ГенераторТестовыхДанных.ПодписчикПодсистемаИнтеграции();
...
Процедура КешированиеПодсистемаИнтеграции_ЗаписьПодписчикаЗаполняетСохраненнуюОбработку() Экспорт
