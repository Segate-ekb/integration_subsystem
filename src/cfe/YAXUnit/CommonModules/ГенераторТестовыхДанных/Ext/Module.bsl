...
Функция ЭндпоинтBasic(Пользователь = "testuser", Пароль = "testpass", АдресРесурса = "https://httpbin.org") Экспорт
	...
	ИТКВ_БСП.ЗаписатьДанныеВБезопасноеХранилище(Эндпоинт, Пользователь, "Basic_Пользователь");
	ИТКВ_БСП.ЗаписатьДанныеВБезопасноеХранилище(Эндпоинт, Пароль, "Basic_Пароль");
...
Функция ЭндпоинтBearer(Токен = "test_bearer_token_12345", АдресРесурса = "https://httpbin.org") Экспорт
	...
	УстановитьПривилегированныйРежим(Истина);
	ИТКВ_БСП.ЗаписатьДанныеВБезопасноеХранилище(Эндпоинт, Токен, "Bearer_Токен");
	УстановитьПривилегированныйРежим(Ложь);
...
Функция ЭндпоинтApiKey(ИмяЗаголовка = "X-API-Key", Значение = "test_api_key_67890", АдресРесурса = "https://httpbin.org") Экспорт
	...
	УстановитьПривилегированныйРежим(Истина);
	ИТКВ_БСП.ЗаписатьДанныеВБезопасноеХранилище(Эндпоинт, ИмяЗаголовка, "ApiKey_ИмяЗаголовка");
	ИТКВ_БСП.ЗаписатьДанныеВБезопасноеХранилище(Эндпоинт, Значение, "ApiKey_Значение");
	УстановитьПривилегированныйРежим(Ложь);
...
Процедура ЗаполнитьТестовыеУчетныеДанные(Эндпоинт, ТипАвторизации)
	...
	Если ТипАвторизации = Перечисления.инт_ТипыАвторизации.Basic Тогда
		ИТКВ_БСП.ЗаписатьДанныеВБезопасноеХранилище(Эндпоинт, "test_user", "Basic_Пользователь");
		ИТКВ_БСП.ЗаписатьДанныеВБезопасноеХранилище(Эндпоинт, "test_password", "Basic_Пароль");
		
	ИначеЕсли ТипАвторизации = Перечисления.инт_ТипыАвторизации.Digest Тогда
		ИТКВ_БСП.ЗаписатьДанныеВБезопасноеХранилище(Эндпоинт, "digest_user", "Digest_Пользователь");
		ИТКВ_БСП.ЗаписатьДанныеВБезопасноеХранилище(Эндпоинт, "digest_password", "Digest_Пароль");
		
	ИначеЕсли ТипАвторизации = Перечисления.инт_ТипыАвторизации.Bearer Тогда
		ИТКВ_БСП.ЗаписатьДанныеВБезопасноеХранилище(Эндпоинт, "test_bearer_token", "Bearer_Токен");
		
	ИначеЕсли ТипАвторизации = Перечисления.инт_ТипыАвторизации.ApiKey Тогда
		ИТКВ_БСП.ЗаписатьДанныеВБезопасноеХранилище(Эндпоинт, "X-API-Key", "ApiKey_ИмяЗаголовка");
		ИТКВ_БСП.ЗаписатьДанныеВБезопасноеХранилище(Эндпоинт, "test_api_key_value", "ApiKey_Значение");
		
	ИначеЕсли ТипАвторизации = Перечисления.инт_ТипыАвторизации.Tls Тогда
		ИТКВ_БСП.ЗаписатьДанныеВБезопасноеХранилище(Эндпоинт, "C:\certs\test.pfx", "Tls_СертификатПуть");
		ИТКВ_БСП.ЗаписатьДанныеВБезопасноеХранилище(Эндпоинт, "cert_password", "Tls_СертификатПароль");
	КонецЕсли;
