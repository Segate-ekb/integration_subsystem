...
Функция ПотокДанныхИсходящий(Активен = Истина,КоличествоПопытокОбработки =0, ПаузаМеждуПопыткамиОбработки=0, Валидация = Ложь, ИдентификаторПотока = "", Знач ТекстОбработчика = "") Экспорт
...
// Функция - Поток данных исходящий с меткой в обработчике
// Обработчик потока кладёт метку в результат. По ней определяется, код какого именно
// потока фактически исполнен.
//
// Параметры:
//  Метка - Строка - Значение, которое обработчик помещает в ключ "метка".
//
// Возвращаемое значение:
//  СправочникСсылка.инт_ПотокиДанных - Записанный поток данных.
//
Функция ПотокДанныхИсходящийСМеткой(Знач Метка) Экспорт
    
    ТекстОбработчика = СтрШаблон("Результат = Новый Соответствие;
    |Результат.Вставить(""метка"", ""%1"");", Метка);
    
    Возврат ПотокДанныхИсходящий(, , , , , ТекстОбработчика);
    
КонецФункции

Функция ПотокДанныхВходящий(Активен = Истина,КоличествоПопытокОбработки =0, ПаузаМеждуПопыткамиОбработки=0, ИдентификаторПотока = "", АсинхроннаяОбработка = Истина, ТекстОбработчика = "") Экспорт
...
// Функция - Текст пост-обработки с меткой
// Возвращает текст обработчика пост-обработки, кладущего метку в ответ подписчика.
//
// Параметры:
//  Метка - Строка - Значение, помещаемое обработчиком в ключ "метка" ответа.
//
// Возвращаемое значение:
//  Строка - Текст обработчика пост-обработки.
//
Функция ТекстПостОбработкиСМеткой(Знач Метка) Экспорт
	
	Возврат СтрШаблон("Ответ.Вставить(""метка"", ""%1"");", Метка);
	
КонецФункции

// Функция - Подписчик с пост-обработкой
// Добавляет подписчику строку табличной части ПостПроцессинг с заданным обработчиком.
//
// Параметры:
//  Подписчик        - СправочникСсылка - Подписчик вида, имеющего табличную часть ПостПроцессинг.
//  КодСостояния     - Число - Код состояния ответа, для которого задан обработчик.
//  ТекстОбработчика - Строка - Текст обработчика пост-обработки.
//
// Возвращаемое значение:
//  СправочникСсылка - Тот же подписчик.
//
Функция ПодписчикСПостОбработкой(Знач Подписчик, Знач КодСостояния, Знач ТекстОбработчика) Экспорт
	
	Объект = Подписчик.ПолучитьОбъект();
	СтрокаПостПроцессинга = Объект.ПостПроцессинг.Добавить();
	СтрокаПостПроцессинга.КодСостояния = КодСостояния;
	СтрокаПостПроцессинга.Обработчик = ТекстОбработчика;
	Объект.Записать();
...
// Функция - Ответ подписчика
// Собирает ответ в том виде, в каком его получает пост-обработка от коннектора HTTP.
//
// Параметры:
...
//
// Возвращаемое значение:
//  Структура - КодСостояния, Тело.
//
Функция ОтветПодписчика(Знач КодСостояния, Знач Тело = "") Экспорт
	
	Возврат Новый Структура("КодСостояния, Тело",
		КодСостояния,
		ПолучитьДвоичныеДанныеИзСтроки(Тело, "UTF-8", Ложь));
...
//  ИспользоватьКакКонсьюмер - Булево - Режим консьюмера
//  ИмяDLQТопика - Строка - Имя DLQ-топика (обязательно для консьюмера)
//
// Возвращаемое значение:
//  СправочникСсылка.инт_ПодписчикиKafka - Ссылка на созданного подписчика
//
Функция ПодписчикKafka(Эндпоинт = Неопределено, ИмяТопика = "test-topic", ПакетнаяОбработка = Ложь, ИспользоватьКакКонсьюмер = Ложь, ИмяDLQТопика = "") Экспорт
	...
	Конструктор = ЮТест.Данные().КонструкторОбъекта(Справочники.инт_ПодписчикиKafka)
		...
		.Установить("КоличествоПопытокОтправки", 3)
		.Установить("ПаузаМеждуПопыткамиОбработки", 0)
		.Установить("ИспользоватьКакКонсьюмер", ИспользоватьКакКонсьюмер)
		.Установить("ИмяDLQТопика", ИмяDLQТопика);
...
// Функция - Часовой пояс, отличный от часового пояса информационной базы
// Проверки пространства времени имеют смысл только при несовпадении часовых поясов сеанса
// и информационной базы: при совпадении смещение равно нулю и дефект не проявляется.
//
// Возвращаемое значение:
//  Строка - Имя часового пояса, смещение которого заведомо отличается от смещения базы
//
Функция ЧасовойПоясОтличныйОтПоясаБазы() Экспорт

    СмещениеБазыСек = инт_ОбщегоНазначения.ВремяБазы() - ТекущаяУниверсальнаяДата();

    Возврат ?(СмещениеБазыСек > 21600, "UTC", "Asia/Kamchatka");
