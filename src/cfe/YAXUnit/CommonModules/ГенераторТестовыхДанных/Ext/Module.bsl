...
// Функция - Часовой пояс, отличный от часового пояса сервера приложений
// Проверки пространства времени имеют смысл только при несовпадении часовых поясов сеанса
// и сервера приложений: при совпадении смещение равно нулю и дефект не проявляется.
//
// Возвращаемое значение:
//  Строка - Имя часового пояса, смещение которого заведомо отличается от смещения сервера
//
Функция ЧасовойПоясОтличныйОтСерверного() Экспорт

    // Местное время сервера приложений — единственный источник его смещения.
    СмещениеСервераСек = ТекущаяДата() - ТекущаяУниверсальнаяДата();

    Возврат ?(СмещениеСервераСек > 21600, "UTC", "Asia/Kamchatka");
