...
Процедура ИсполняемыеСценарии() Экспорт
    ...
    ЮТТесты.УдалениеТестовыхДанных().ВТранзакции()
		...
		.ДобавитьТест("ФормированиеБезЗаписиВОчередиОтклоняется")
...
// Без записи в очереди незаполненное сообщение сериализуется в null, поэтому формирование должно отклоняться.
Процедура ФормированиеБезЗаписиВОчередиОтклоняется() Экспорт

    Сообщение = Новый Структура("ИдентификаторСообщения, Подписчик",
        Новый УникальныйИдентификатор, ГенераторТестовыхДанных.Подписчик());

    ЮТест.ОжидаетЧто(РегистрыСведений.инт_ОчередьОтправкиИсходящихСообщений)
        .Метод("СформироватьСообщение")
        .Параметр(Сообщение)
        .ВыбрасываетИсключение("отсутствует в очереди исходящих");

    ЮТест.ОжидаетЧтоТаблицаБазы("РегистрСведений.инт_ОчередьОтправкиИсходящихСообщений")
        .НеСодержитЗаписи(ЮТест.Предикат().Реквизит("ИдентификаторСообщения").Равно(Сообщение.ИдентификаторСообщения));
...
Процедура ТестФормированияСообщения(ТипПодписчика, ЭталонноеСообщение) Экспорт
    ...
    Мокито.Обучение(РегистрыСведений.инт_ОчередьИсходящихСообщений)
           .Когда("ПолучитьДанныеОчередиПоИдентификатору").Вернуть(Новый Структура("СформированноеСообщение,ПотокДанных", Неопределено, ГенераторТестовыхДанных.ПотокДанныхИсходящий()))
           .Обучение(инт_ПодписчикиОбщий)
...
Процедура ТестОтправкиСообщения() Экспорт
     ...
     Мокито.Обучение(инт_ПодписчикиОбщий)
