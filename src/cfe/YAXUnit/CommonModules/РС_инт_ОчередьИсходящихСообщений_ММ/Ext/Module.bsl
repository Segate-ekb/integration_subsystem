// BSLLS-off
// @strict-types

/////////////////////////////////////////////////////////////////////////////////
// Экспортные процедуры и функции, предназначенные для использования другими 
// объектами конфигурации или другими программами
///////////////////////////////////////////////////////////////////////////////// 

#Область СлужебныйПрограммныйИнтерфейс

Процедура ИсполняемыеСценарии() Экспорт
    
    ВариантыПолученияДанныхОчереди = ЮТест.Варианты("ПредставлениеТеста, СписокПолей")
    .Добавить(
      "Одно поле",
      "ИсходныеДанные")
    .Добавить(
      "Несколько полей",
      "ИсходныеДанные,ПотокДанных")
    .Добавить(
      "Несколько полей с пробелами",
      "ИсходныеДанные, ПотокДанных")
    .Добавить(
      "Получение сообщения из хз",
      "СформированноеСообщение")
    .Добавить(
      "Пустая строка",
      "");

  Тесты =  ЮТТесты.УдалениеТестовыхДанных().ВТранзакции()
    	.ДобавитьТест("РегистрацияСообщенияВОчередь")
        .ДобавитьТест("ВходящийПотокНеПопадаетВОчередь")
        .ДобавитьТест("НеактивныйПотокНеПопадаетВОчередь")
        .ДобавитьТест("ФормированиеСообщенияПоПотоку")
        .ДобавитьТест("ПроверкаРассылкиПодписчикам")
        .ДобавитьТест("ПовторнаяРегистрацияНеизмененногоОбъектаНеСоздаетДубль")
        .ДобавитьТест("ДанныеОчередиБезЗаписиВозвращаютНезаполненныеЗначения");
    Для Каждого Вариант Из ВариантыПолученияДанныхОчереди.СписокВариантов() Цикл
        Тесты.ДобавитьТест("ПолучениеДанныхОчередиПоИдентификатору")
                .Представление(Вариант.ПредставлениеТеста, Истина)
                .СПараметрами(Вариант.СписокПолей);
	
    КонецЦикла;

    ВариантыУдаленныхПолей = ЮТест.Варианты("ПредставлениеТеста, ВидПоля")
    .Добавить(
      "Удален реквизит",
      "Реквизит")
    .Добавить(
      "Удалена колонка табличной части",
      "КолонкаТабличнойЧасти")
    .Добавить(
      "Удалена табличная часть",
      "ТабличнаяЧасть")
    .Добавить(
      "Удалены все колонки табличной части",
      "ВсеКолонкиТабличнойЧасти");
    Для Каждого Вариант Из ВариантыУдаленныхПолей.СписокВариантов() Цикл
        Тесты.ДобавитьТест("РегистрацияПослеУдаленияОтслеживаемогоПоляИзМетаданных")
                .Представление(Вариант.ПредставлениеТеста, Истина)
                .СПараметрами(Вариант.ВидПоля);

    КонецЦикла;

КонецПроцедуры

#Область События

Процедура ПередВсемиТестами() Экспорт

КонецПроцедуры

Процедура ПередКаждымТестом() Экспорт

КонецПроцедуры

Процедура ПослеКаждогоТеста() Экспорт

КонецПроцедуры

Процедура ПослеВсехТестов() Экспорт

КонецПроцедуры

#КонецОбласти

Процедура РегистрацияСообщенияВОчередь() Экспорт
    ИсходныеДанные = ГенераторТестовыхДанных.демо_Документ();
    ПотокДанных = ГенераторТестовыхДанных.ПотокДанныхИсходящий();
    ИдентификаторСообщения = РегистрыСведений.инт_ОчередьИсходящихСообщений.ЗарегистрироватьСообщение(ИсходныеДанные, ПотокДанных);
    
    ЮТест.ОжидаетЧто(ИдентификаторСообщения).НеРавно(Неопределено);
    
    УсловиеПоискаЗаписи = ЮТест.Предикат() // В базе есть записаное сообщение в очереди со статусом "Новый"
            .Реквизит("ИдентификаторСообщения").Равно(ИдентификаторСообщения);

    ЮТест.ОжидаетЧтоТаблицаБазы("РегистрСведений.инт_ОчередьИсходящихСообщений")
        .СодержитЗаписи(УсловиеПоискаЗаписи);
        
    ЗаписьСообщения = ЮТЗапросы.Запись("РегистрСведений.инт_ОчередьИсходящихСообщений", УсловиеПоискаЗаписи);
    ЮТест.ОжидаетЧто(ЗаписьСообщения, "Запись сообщения")
        .Заполнено()
            .Свойство("ИсходныеДанные").Заполнено().Равно(ЗначениеВСтрокуВнутр(ИсходныеДанные))
            .Свойство("ПотокДанных").Заполнено().Равно(ПотокДанных);

    ЮТест.ОжидаетЧтоТаблицаБазы("РегистрСведений.инт_ТекущийСтатусИсходящихСообщений")
        .СодержитЗаписи(УсловиеПоискаЗаписи);
        
    ЗаписьСтатуса = ЮТЗапросы.Запись("РегистрСведений.инт_ТекущийСтатусИсходящихСообщений", УсловиеПоискаЗаписи);
    ЮТест.ОжидаетЧто(ЗаписьСтатуса, "Запись статуса")
        .Заполнено()
            .Свойство("СтатусСообщения").Заполнено().Равно(Перечисления.инт_СтатусыИсходящихСообщений.Новый);
    
КонецПроцедуры
        
Процедура ВходящийПотокНеПопадаетВОчередь() Экспорт
	ИсходныеДанные = ГенераторТестовыхДанных.демо_Документ();
    ПотокДанных = ГенераторТестовыхДанных.ПотокДанныхВходящий();
    ИдентификаторСообщения = РегистрыСведений.инт_ОчередьИсходящихСообщений.ЗарегистрироватьСообщение(ИсходныеДанные, ПотокДанных);

    ЮТест.ОжидаетЧто(ИдентификаторСообщения).Равно(Неопределено);

КонецПроцедуры
    
Процедура НеактивныйПотокНеПопадаетВОчередь() Экспорт
	ИсходныеДанные = ГенераторТестовыхДанных.демо_Документ();
    ПотокДанных = ГенераторТестовыхДанных.ПотокДанныхИсходящий(Ложь);
    ИдентификаторСообщения = РегистрыСведений.инт_ОчередьИсходящихСообщений.ЗарегистрироватьСообщение(ИсходныеДанные, ПотокДанных);

    ЮТест.ОжидаетЧто(ИдентификаторСообщения).Равно(Неопределено);

КонецПроцедуры
    
Процедура ФормированиеСообщенияПоПотоку() Экспорт
	ИсходныеДанные = ГенераторТестовыхДанных.демо_Документ();
    ПотокДанных = ГенераторТестовыхДанных.ПотокДанныхИсходящий();
    // Проверка регистрации выполняется другим тестом.
    ИдентификаторСообщения = РегистрыСведений.инт_ОчередьИсходящихСообщений.ЗарегистрироватьСообщение(ИсходныеДанные, ПотокДанных);
    
    ЮТест.ОжидаетЧто(ИдентификаторСообщения).НеРавно(Неопределено);
    
    СформированноеСообщение = Новый Соответствие;
    СформированноеСообщение.Вставить("Тест", "Тест");
    
    Мокито.Обучение(РегистрыСведений.инт_ОчередьИсходящихСообщений)
            .Когда("ПолучитьДанныеОчередиПоИдентификатору").Вернуть(Новый Структура("ИсходныеДанные,ПотокДанных"))
            .Обучение(Справочники.инт_ПотокиДанных, Истина)
            .Когда("СформироватьСообщениеПоПотоку").Вернуть(СформированноеСообщение)
     .Прогон();
    
    РегистрыСведений.инт_ОчередьИсходящихСообщений.СформироватьСообщениеПоИдентификатору(ИдентификаторСообщения);
    
    Мокито.Сбросить();

    УсловиеПоискаЗаписи = ЮТест.Предикат()
            .Реквизит("ИдентификаторСообщения").Равно(ИдентификаторСообщения);
    ЮТест.ОжидаетЧтоТаблицаБазы("РегистрСведений.инт_ОчередьИсходящихСообщений")
        .СодержитЗаписи(УсловиеПоискаЗаписи);
        
    ЗаписьСообщения = ЮТЗапросы.Запись("РегистрСведений.инт_ОчередьИсходящихСообщений", УсловиеПоискаЗаписи);
    ЮТест.ОжидаетЧто(ЗаписьСообщения, "Запись сообщения")
        .Заполнено()
            .Свойство("СформированноеСообщение").Заполнено();
    ДанныеИсходящегоСообщения = РегистрыСведений.инт_ОчередьИсходящихСообщений.ПолучитьДанныеОчередиПоИдентификатору(ИдентификаторСообщения, "СформированноеСообщение");
    ЮТест.ОжидаетЧто(ДанныеИсходящегоСообщения.СформированноеСообщение).Равно(СформированноеСообщение);
            
   ЮТест.ОжидаетЧтоТаблицаБазы("РегистрСведений.инт_ТекущийСтатусИсходящихСообщений")
        .СодержитЗаписи(УсловиеПоискаЗаписи);
        
    ЗаписьСтатуса = ЮТЗапросы.Запись("РегистрСведений.инт_ТекущийСтатусИсходящихСообщений", УсловиеПоискаЗаписи);
    ЮТест.ОжидаетЧто(ЗаписьСтатуса, "Запись Статуса")
        .Заполнено()
            .Свойство("СтатусСообщения").Заполнено().Равно(Перечисления.инт_СтатусыИсходящихСообщений.ГотовоКОтправке);

КонецПроцедуры

Процедура ПолучениеДанныхОчередиПоИдентификатору(СписокПолей) Экспорт

   	ИсходныеДанные = ГенераторТестовыхДанных.демо_Документ();
    ПотокДанных = ГенераторТестовыхДанных.ПотокДанныхИсходящий();
    
    // Проверка регистрации выполняется другим тестом.
    ИдентификаторСообщения = РегистрыСведений.инт_ОчередьИсходящихСообщений.ЗарегистрироватьСообщение(ИсходныеДанные, ПотокДанных);
    
    МассивПолей = СтрРазделить(СписокПолей,",",Ложь);
    Если МассивПолей.Количество()=0 Тогда
        ЮТест.ОжидаетЧто(РегистрыСведений.инт_ОчередьИсходящихСообщений)
        .Метод("ПолучитьДанныеОчередиПоИдентификатору")
        .Параметр(ИдентификаторСообщения)
        .Параметр(СписокПолей)
        .ВыбрасываетИсключение("Список полей - не может быть пустым!");
    Иначе
        СтруктураРезультат = РегистрыСведений.инт_ОчередьИсходящихСообщений.ПолучитьДанныеОчередиПоИдентификатору(ИдентификаторСообщения, СписокПолей);
        Для Каждого Поле Из МассивПолей Цикл
            ЮТест.ОжидаетЧто(СтруктураРезультат)
            .Свойство(СокрЛП(Поле));
            Если Поле="СформированноеСообщение" Тогда
                ЮТест.ОжидаетЧто(СтруктураРезультат)
                .Свойство(СокрЛП(Поле)).НеИмеетТип(Тип("ХранилищеЗначения"));
            КонецЕсли;
        КонецЦикла;
    КонецЕсли;
КонецПроцедуры

// Контракт функции: ключи структуры есть всегда, отсутствие записи видно по незаполненным значениям.
Процедура ДанныеОчередиБезЗаписиВозвращаютНезаполненныеЗначения() Экспорт

    СписокПолей = "ИсходныеДанные, ПотокДанных, СформированноеСообщение";

    СтруктураРезультат = РегистрыСведений.инт_ОчередьИсходящихСообщений.ПолучитьДанныеОчередиПоИдентификатору(
        Новый УникальныйИдентификатор, СписокПолей);

    Проверка = ЮТест.ОжидаетЧто(СтруктураРезультат, "Данные сообщения вне очереди");
    Для Каждого Поле Из СтрРазделить(СписокПолей, ",", Ложь) Цикл
        Проверка.Свойство(СокрЛП(Поле)).НеЗаполнено();
    КонецЦикла;

КонецПроцедуры

Процедура ПроверкаРассылкиПодписчикам() Экспорт
	ИсходныеДанные = ГенераторТестовыхДанных.демо_Документ();
    ПотокДанных = ГенераторТестовыхДанных.ПотокДанныхИсходящий();
    
    // Проверка регистрации выполняется другим тестом.
    ИдентификаторСообщения = РегистрыСведений.инт_ОчередьИсходящихСообщений.ЗарегистрироватьСообщение(ИсходныеДанные, ПотокДанных);
    
    ЮТест.ОжидаетЧто(РегистрыСведений.инт_ОчередьИсходящихСообщений)
        .Метод("ЗарегистрироватьСообщениеКОтправке")
            .Параметр(ИдентификаторСообщения)
            .НеВыбрасываетИсключение();
    
КонецПроцедуры

Процедура ПовторнаяРегистрацияНеизмененногоОбъектаНеСоздаетДубль() Экспорт
    ИсходныеДанные = ГенераторТестовыхДанных.демо_Документ();
    ПотокДанных = ГенераторТестовыхДанных.ПотокДанныхИсходящий();

    РегистрыСведений.инт_ОчередьИсходящихСообщений.ЗарегистрироватьСообщение(ИсходныеДанные, ПотокДанных);
    ИдентификаторДубля = РегистрыСведений.инт_ОчередьИсходящихСообщений.ЗарегистрироватьСообщение(ИсходныеДанные, ПотокДанных);

    ЮТест.ОжидаетЧто(ИдентификаторДубля, "Идентификатор повторной регистрации").Равно(Неопределено);
КонецПроцедуры

Процедура РегистрацияПослеУдаленияОтслеживаемогоПоляИзМетаданных(ВидПоля) Экспорт
    ИсходныеДанные = ГенераторТестовыхДанных.демо_Документ();
    ПотокДанных = ГенераторТестовыхДанных.ПотокДанныхИсходящий();

    // Хеш сохранен по прежнему составу реквизитов.
    РегистрыСведений.инт_ОчередьИсходящихСообщений.ЗарегистрироватьСообщение(ИсходныеДанные, ПотокДанных);

    ЭмулироватьУдалениеПоляИзМетаданных(ПотокДанных, ВидПоля);

    ИдентификаторСообщения = РегистрыСведений.инт_ОчередьИсходящихСообщений.ЗарегистрироватьСообщение(ИсходныеДанные, ПотокДанных);
    ЮТест.ОжидаетЧто(ИдентификаторСообщения, "Идентификатор сообщения после изменения состава реквизитов").НеРавно(Неопределено);

    ИдентификаторДубля = РегистрыСведений.инт_ОчередьИсходящихСообщений.ЗарегистрироватьСообщение(ИсходныеДанные, ПотокДанных);
    ЮТест.ОжидаетЧто(ИдентификаторДубля, "Идентификатор повторной регистрации").Равно(Неопределено);
КонецПроцедуры

#КонецОбласти

#Область СлужебныеПроцедурыИФункции

// Переименовывает отслеживаемые поля потока так, что в метаданных объекта таких полей нет.
// Это состояние потока после удаления реквизита из объекта: хеш посчитан по прежнему составу,
// а имена удаленных полей остались в отслеживаемых реквизитах.
//
// Параметры:
//  ПотокДанных - СправочникСсылка.инт_ПотокиДанных - Поток данных, у которого меняются отслеживаемые реквизиты
//  ВидПоля     - Строка - Реквизит, КолонкаТабличнойЧасти, ВсеКолонкиТабличнойЧасти или ТабличнаяЧасть
//
Процедура ЭмулироватьУдалениеПоляИзМетаданных(ПотокДанных, ВидПоля)
    ОтслеживаемыеРеквизиты = ЗначениеИзСтрокиВнутр(ПотокДанных.ОтслеживаемыеРеквизиты);
    СтрокиОбъекта = ОтслеживаемыеРеквизиты.Строки[0].Строки;

    УдаленныеПоля = Новый Массив;
    Если ВидПоля = "Реквизит" Тогда
        УдаленныеПоля.Добавить(СтрокиОбъекта.Найти("Реквизиты", "Имя", Ложь).Строки.Найти("ДемоРеквизитСтрока", "Имя", Ложь));
    ИначеЕсли ВидПоля = "КолонкаТабличнойЧасти" Тогда
        УдаленныеПоля.Добавить(СтрокаТабличнойЧастиТовары(СтрокиОбъекта).Строки.Найти("Цена", "Имя", Ложь));
    ИначеЕсли ВидПоля = "ВсеКолонкиТабличнойЧасти" Тогда
        Для Каждого Колонка Из СтрокаТабличнойЧастиТовары(СтрокиОбъекта).Строки Цикл
            УдаленныеПоля.Добавить(Колонка);
        КонецЦикла;
    Иначе
        УдаленныеПоля.Добавить(СтрокаТабличнойЧастиТовары(СтрокиОбъекта));
    КонецЕсли;

    Для Каждого УдаленноеПоле Из УдаленныеПоля Цикл
        УдаленноеПоле.Имя = УдаленноеПоле.Имя + "Удаленное";
    КонецЦикла;

    ОбъектПотока = ПотокДанных.ПолучитьОбъект();
    ОбъектПотока.ОтслеживаемыеРеквизиты = ЗначениеВСтрокуВнутр(ОтслеживаемыеРеквизиты);
    ОбъектПотока.Записать();
КонецПроцедуры

Функция СтрокаТабличнойЧастиТовары(СтрокиОбъекта)
    Возврат СтрокиОбъекта.Найти("ТабличныеЧасти", "Имя", Ложь).Строки.Найти("Товары", "Имя", Ложь);
КонецФункции

#КонецОбласти
